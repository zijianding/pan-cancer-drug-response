...
filter_mRNA <- function( data, train_pats, low.thresh="Q1", type="dispersion" )                        
{
...
exp_norm <- function(data, train_pats, info)
{
  ##input
...
test_gene <- function(train_dat, info, sig_gene = 500,
                      p_thresh=0.05,q_thresh=0.05, type="ttest")
{
  ...
  if( type=="ttest" )
  {
    ...
    if( q_use == TRUE )
    {
      ...
      return(list(train_dat,"q_thresh",q_thresh))
    }
    if( (q_use==F) && (p_use==T) )
    {
      ...
      return(list(train_dat,"p_thresh",p_thresh))
    }
    if( (q_use==F) && (p_use==F) )
    {
      ...
      return(list(train_dat,"sig_gene",sig_gene))
    }
...
test_type = args[5] #ttest / wilcox / regression
#desktop
...
list_tmp = test_gene(train.dat, cisplatin.info, type="regression")
train.dat = list_tmp[[1]]
test_type = list_tmp[[2]]
thresh = list_tmp[[3]]
...
cancer_dummy = tmp_list[[2]]
#test.dat = dummy_to_test(test.dat, cisplatin.info, cancer_dummy)
...
for( bs in 1:BS  )
{
  ...
  for( i in 1:length(alphas) )
  {
    ...
    cv_fit = cv.glmnet( t(curr.train_dat), as.factor(curr.train_resp), family="binomial",
                        type.measure="auc",alpha=alphas[i], parallel=TRUE)
    stopImplicitCluster()
    stopCluster(cl)
