...
# score.mat = read.table("../../../results/omics_features/cnv/cisplatin/elastic_net/pooled.test_1.elanet.score_mat_CV.txt",
...
#record the best models(PR+ROC) CV performance
...
#这下面的index是不是有问题，感觉应该是1+length(core.cancer)+1才对
...
for(ca in 1:length(core.cancer))
{
  ...
  auc.score <- foreach(i=1:nrow(mat.params),.combine='c') %dopar%
  {
    ...
    row_ix = match(mat.params[i,1],alphas)
    col_ix = match(mat.params[i,2],lambdas)
    if( (df.mat[row_ix,col_ix] <= leat_feat) || (df_freq.mat[row_ix,col_ix]>=null_model) )
    {
      return(0)
    }
    if( (df.mat[row_ix,col_ix] > leat_feat) && (df_freq.mat[row_ix,col_ix]<null_model) )
  ...
  best.classify_score_cv[ca+1+length(core.cancer)+1,] = score.mat[curr_auc.best_ix,]
...
mat.feature_hitting = array(NA,c(nrow(train.dat),100,8))
...
for(bs in 1:BS)
{
  ...
  curr.train_dat = as.matrix( t( cisplatin.dat[,match(bs_mat.pats[,bs],as.character(colnames(cisplatin.dat)))] ) )
  for(j in 1:ncol(curr.train_dat))
  {
    curr.train_dat[which(is.na(curr.train_dat[,j])),j] = mean(curr.train_dat[,j],na.rm=T)
  }
  ...
  for(i in 1:(1+length(core.cancer)))
  {
    ...
    if( levels(as.factor(curr.train_resp))[2] != "insensitive" )
    {
      test_score[,bs,i+1+length(curr.train_resp)] = 1 - test.res
    }
  }
    
}
test_score.ave = matrix(0,nrow=length(test.pats),ncol=8)
...
mat.feature = matrix(0,nrow=nrow(mat.feature_hitting),ncol=8)
...
for(ca in 1:length(core.cancer))
{
  ...
  tmp_str = paste("Pan-cancer(pooled) chosen by",core.cancer[ca],"(TEST)",sep=" ")
  plot(prauc.best,main=tmp_str);
  legend("topright",legend=paste("alpha=",best.param_pr[ca+1,1],";lambda=",best.param_pr[ca+1,2],sep=""))
