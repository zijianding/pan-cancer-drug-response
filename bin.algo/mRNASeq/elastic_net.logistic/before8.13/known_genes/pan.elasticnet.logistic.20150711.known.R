...
rna_map_genes <- function(rna_genes,known_genes)
{
  tmp_list = strsplit(rna_genes,"\\|")
  genes = unlist( lapply(tmp_list,function(x) return(x[1])) )
  common_genes = intersect( genes,unique(toupper(known_genes)) )
  ix = match(common_genes,genes)
  if(length(ix)>0)
  {
    return(ix)
  }
...
known_genes = read.table(args[6],header=F,quote="",row.names=NULL)
diff_gene = as.character(args[7]) #"diff_gene"
add_cancer = as.character(args[8]) # "add_cancer"
elastic_net = as.character(args[9]) # "elastic_net"
#desktop
...
# output_folder = "."
# freq = 0.8
# known_genes = read.table("C:/Users/zding/workspace/projects/drug_sensitivity/data/gene_sets/hanre.seed_gene.txt",
#                          header=F,quote="",row.names=NULL)
#both
...
#filter genes
gene.ix = rna_map_genes(row.names(train.dat),known_genes$V1)
train.dat = train.dat[gene.ix,]
test.dat = test.dat[gene.ix,]
