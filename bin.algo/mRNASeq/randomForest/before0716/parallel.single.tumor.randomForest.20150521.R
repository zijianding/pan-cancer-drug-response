...
{
  na_num = length( which(is.na(test.dat[i,])) )
  if( na_num > 0)
  {
    test.dat[i,which(is.na(test.dat[i,]))] = mean(as.numeric(as.character(test.dat[i,])),na.rm=T)
  }
}
...
for(fold in 1:folds)
{
  ...
  for(j in 1:ncol(curr.validation_dat))
  {
    curr.validation_dat[which(is.na(curr.validation_dat[,j])),j] = mean( curr.validation_dat[,j],na.rm=T )
  }
  #impute NAs by means
  ...
  #impute NAs by means
  for(j in 1:ncol(curr.train_dat))
  {
    curr.train_dat[which(is.na(curr.train_dat[,j])),j] = mean(curr.train_dat[,j],na.rm=T)
  }
...
# actually for debug
tmp_str = paste(output_folder,"/","single.",core.cancer,
                ".randomForest.test_",test_fold-3,".score_mat(CV).txt",sep="")
write.table(score.mat,tmp_str,col.names=T,row.names=T,quote=F,sep="\t")


####best model performance of CV####
file_name = paste(output_folder,"/","performance.randomForest.single_",core.cancer,".test_",test_fold-3,".20150526.pdf",sep="")
pdf(file=file_name)
#store CV performance
cv_score.pr_roc = matrix(0,nrow=2,ncol=length(train.pats))
dimnames(cv_score.pr_roc) = list(c("pr","roc"),train.pats)
...
cv_score.pr_roc[1,] = score.mat[auc.best_ix,]
...
cv_score.pr_roc[2,] = score.mat[auc.best_ix,]
...
#the next for loop, DO NOT parallel
for(bs in 1:BS)
{
  #make even the class
  ...
  rf_fit.pr = randomForest(x=curr.train_dat,y=as.factor(curr.train_resp),ntree=BS,
                        strata=as.factor(curr.train_resp),mtry=floor(pr_best*ncol(curr.train_dat)),
                        sampsize=c(sample.num,sample.num),importance=T, na.action="na.omit")
  ...
  rf_fit.roc = randomForest(x=curr.train_dat,y=as.factor(curr.train_resp),ntree=BS,
                        strata=as.factor(curr.train_resp),mtry=floor(roc_best*ncol(curr.train_dat)),
                        sampsize=c(sample.num,sample.num),importance=T, na.action="na.omit" )
...
#output the best CV performance
tmp_str = paste(output_folder,"/","single.",core.cancer,".randomForest.cv_score.test_",test_fold-3,".txt",sep="")
...
tmp_str = paste(output_folder,"/","single.",core.cancer,".randomForest.test_score.test_",test_fold-3,".txt",sep="")
...
rownames(gini) = rownames(train.dat)
tmp_str = paste(output_folder,"/","single.",core.cancer,
                ".randomForest.feature_stat.test_",test_fold-3,".txt",sep="")
write.table(gini,file=tmp_str,row.names=T,col.names=T,sep="\t",quote=F)

#output parameters of best models
best_param.vec = matrix(c(pr_best,roc_best),nrow=2)
rownames(best_param.vec) = c("pr","roc")
tmp_str = paste(output_folder,"/","single.",core.cancer,
                ".randomForest.best_parameters.test_",test_fold-3,".txt",sep="")
write.table(best_param.vec,file=tmp_str,row.names=T,col.names=T,sep="\t",quote=F)
