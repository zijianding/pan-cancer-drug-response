...
test_fold = as.numeric(as.character(args[6]))#the current test fold, ranging from 1 to 100
shuffle = as.numeric(as.character(args[7])) #"NULL"/1-100

input_type = args[8]   # molecular_only/clinical_only/clinical_molecular/***/***
output_type = args[9]  # "performance" for ROC, "marker" for extract marker, "shuffle" for permutation
calc_cancer = args[10]  # "sin_cancer"/"pan_cancer"
calc_gene = args[11]   # "all_gene"/"gene_set"

core.cancer = args[12] # "BLCA" etc/NULL
test.cancer = args[13] # test cancer of cross tumor
gene_set = args[14]    # NULL/gene_set
...
#preprocess data
probes = cisplatin.dat[,1]
cisplatin.dat = cisplatin.dat[,-1]
rownames(cisplatin.dat) = probes
...
if( calc_gene == "gene_set" )
{
  ...
  probe2gene = read.table(probe_file,header=F,sep="\t",quote="")
  cisplatin.dat = map_methy_gene(cisplatin.dat,pre_genes$V1,probe2gene)
}
