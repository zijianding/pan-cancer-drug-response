...
###load data###
#lap-top#
# sin_path = "C:/Users/zding/workspace/projects/drug_sensitivity/results/omics_feature/cnv/cisplatin/randomForest/single/CESC/"
...
file_pattern = args[5]
#both#
sin.files = list.files( path=sin_path,full.names=T, pattern=file_pattern )
...
sin_score = lapply(sin_score,fun_tp)

# add_lbs <- function(x){
#   pats = names(x)
#   ix = match(pats,as.character(cisplatin.info$patient))
#   resp = as.character(cisplatin.info$response[ix])
#   return(resp)
# }
# sin_labels = lapply(sin_score,add_lbs)

# pred = prediction(sin_score,sin_labels)
# perf = performance(pred,"tpr","fpr")
# plot(perf,avg='vertical',spread.estimate='stderror')
cl = makeCluster(no_cores)
registerDoParallel(cl)
roc_res <- foreach(i=1:length(sin_score) ) %dopar%
{
...
roc = ave_roc(roc_res,type="vertical",measure="sd",
              paste("Performance on",core.cancer,"based on CNV") )
title(NULL,"Vertical average;std bar")
roc = ave_roc(roc_res,type="vertical",measure="quantile",
              paste("Performance on",core.cancer,"based on CNV") )
title(NULL,"Vertical average;quantile bar")
roc = ave_roc(roc_res,type="threshold",measure="sd",
              paste("Performance on",core.cancer,"based on CNV") )
title(NULL,"Threshold average;std bar")

#AUCs compared to random
...
cl = makeCluster(no_cores)
registerDoParallel(cl)
error_res <- foreach(i=1:length(sin_score)) %dopar%
{
  curr_pats = names( sin_score[[i]] )
  resp = as.character( cisplatin.info$response[ match( curr_pats, as.character(cisplatin.info$patient) ) ] )
...
error = error_calc(error_res,cisplatin.info,core.cancer)
