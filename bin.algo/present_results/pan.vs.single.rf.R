...
# pan_pattern = "pan.elanet.mid_res.test_[0-9]*.20150713.txt"
# single_pattern = "pan.elanet.mid_res.test_[0-9]*.20150701.txt"
#cluster#
...
pan_roc_res <- foreach( i=1:length(pan_score),
                        .combine='comb', .multicombine=TRUE,
                        .init=list(list(), list())  ) %dopar%
{
  #for glmnet, second class is target class
  curr_pats_all = names(pan_score[[i]])
  ...
  return(list(pan_score[[i]],ensemble_roc(pan_score[[i]],resp,"insensitive")))
}
stopImplicitCluster()
stopCluster(cl)
pan_score = pan_roc_res[[1]]
pan_roc_res = pan_roc_res[[2]]
