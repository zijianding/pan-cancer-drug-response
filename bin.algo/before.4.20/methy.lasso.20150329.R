...
# for( round in 1:5 )
# {
#   sample.size = ncol(patient.data)
#   tmp.arr = 1:sample.size
#   train.ix = sample(tmp.arr,size=round(0.8*sample.size))
#   valid.ix = tmp.arr[-train.ix]
#   sen.train.ix = intersect(train.ix,sen.index)
#   insen.train.ix = intersect(train.ix,insen.index)
#   
#   #t test
#   test.res = matrix(0,nrow=nrow(patient.data),ncol=4)
#   colnames(test.res) = c("sensitive","insensitive","pvalue","bonferroni")
#   rownames(test.res) = rownames(patient.data)
#   
#   # cl = makeCluster(4)
#   # registerDoParallel(cl)
#   # foreach(i=1:nrow(patient.data)) %dopar%
#   for( i in 1:5 )
#   {
#     sen.data = unlist(patient.data[i,sen.train.ix])
#     insen.data = unlist(patient.data[i,insen.train.ix])
...
#   cisplatin.data = t(patient.data[pre.probe.ix,(train.ix)])
#   
...
{
  sen.dat = unlist(luad.dat[i,response.luad=="sensitive"])
  insen.dat = unlist(luad.dat[i,response.luad=="insensitive"])
  curr.res = t.test(sen.dat,insen.dat)
  luad.test[i,3] = curr.res$p.value
  luad.test[i,1] = mean(sen.dat)
  luad.test[i,2] = mean(insen.dat)
  
}
luad.test[,4] = p.adjust(luad.test[,3],method="bonferroni")
print(sum(luad.test[,4]<=0.05))
luad.diff.probe.num = sum(luad.test[,4]<=0.05)


blca.test = matrix(0,nrow=nrow(blca.dat),ncol=4)
colnames(blca.test) = c("sensitive.mean","insensitive.mean","p.value","bonferroni")
rownames(blca.test) = rownames(blca.dat)
for(i in 1:nrow(blca.dat))
{
  sen.dat = unlist(blca.dat[i,response.blca=="sensitive"])
  insen.dat = unlist(blca.dat[i,response.blca=="insensitive"])
  curr.res = t.test(sen.dat,insen.dat)
  blca.test[i,3] = curr.res$p.value
  blca.test[i,1] = mean(sen.dat)
  blca.test[i,2] = mean(insen.dat)
  
}
blca.test[,4] = p.adjust(blca.test[,3],method="bonferroni")
print(sum(blca.test[,4]<=0.05))
blca.diff.probe.num = sum(blca.test[,4]<=0.05)

library(glmnet)
if(luad.diff.probe.num >0 )
{
  pre.probe.ix = which(luad.test[,4]<=0.05)
...
if( blca.diff.probe.num > 0 )
{
  pre.probe.ix = which(blca.test[,4]<=0.05)
  blca.dat1 = t(blca.dat[pre.probe.ix,])
  ...
  cv.lasso = cv.glmnet(x=blca.dat1,y=as.factor(response.blca),alpha=1,nfolds=5,type.measure="class",family="binomial")
  #plot(cv.lasso)
  best.lam = cv.lasso$lambda.min
  glm.lasso = glmnet(x=blca.dat1,y=as.factor(response.blca),lambda=cv.lasso$lambda.min,family="binomial",alpha=1)
  
  ###test on LAUD
  luad.dat1 = t(luad.dat1[pre.probe.ix,])
  #estimate NAs using mean values
  for(i in 1:ncol(luad.dat1))
  {
    tmp = which(is.na(luad.dat1[,i]))
  ...
  prediction.res = predict(object = glm.lasso,newx = luad.dat1,s=cv.lasso$lambda.min,type="response")
  prediction.res.arr = vector(mode="character",length=nrow(prediction.res))
  prediction.res.arr[which(prediction.res>=0.5)] = levels(as.factor(response.blca))[2]
  prediction.res.arr[which(prediction.res<0.5)] = levels(response.blca)[1]
  
  mis.rate = sum(prediction.res.arr!=response.luad)
  mis.rate = mis.rate/length(prediction.res.arr)
  mis.rate
  #prediction.res.arr[which(prediction.res.arr!=response.test)]
