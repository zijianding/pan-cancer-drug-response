...
#fit bagging tree models and test
# cl = makeCluster(no_cores)
# registerDoParallel(cl)
# ctrl = trainControl(classProbs = TRUE,
#                     summaryFunction = twoClassSummary)
# bag_tree <- train(x=t(train.dat), y=as.factor(train.resp),
#                   xtest = t(test.dat), ytest = as.factor(test.resp),
#                   method = "rf",metric = "ROC",
#                   trControl = ctrl,ntree = BS,
#                   mtry=feature_num,importance=T,
#                   strata = as.factor(train.resp),
#                   sampsize = rep(sampsize, 2) )
# stopImplicitCluster()
# stopCluster(cl)
...
##final results##
#selected features#
tmp_str = paste(output_folder,"/pan.elanet.feature.test_",test_fold-3,".20150713.txt",sep="")
write.table(rownames(train.dat),tmp_str,row.names=T,col.names=F,quote=F,sep="\t")


#test by each model#
tmp_str = paste(output_folder,"/pan.elanet.mid_res.test_",test_fold-3,".20150713.txt",sep="")
write.table(pred,tmp_str,col.names=T,row.names=T,sep="\t",quote=F)


##plot TEST performance##
tmp_str = paste(output_folder,"/pan.elanat.test_",test_fold-3,".20150713.tiff",sep="")
tiff(tmp_str)
#roc
