...
freq = args[5] #should be a adjustble parameter
...
train_list <- foreach( bs=1:BS,.packages="glmnet"  ) %dopar%
{
...
best_betas = do.call(cbind,train_list)
rownames(best_betas) = rownames(train.dat)

list_features = gene_selection(t(best_betas),freq=freq)
