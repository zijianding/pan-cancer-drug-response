...
colnames(param_mat) = c("cost","gamma")


BS = 100
best.param_pr = matrix(0,nrow=2,ncol=length(core.cancer))
best.param_roc = matrix(0,nrow=2,ncol=length(core.cancer))
#colnames(best.lambdas) = best.lambdas

#all.res = list()
pdf("performance.svm.gauss.single_cancer.20150513.pdf")
for(ca in 1:length(core.cancer))
{
  ...
  for(cv in 4:8)
  {
    ...
    for(bs in 1:BS)
    {
      ...
      for(j in 1:nrow(param_mat))
      {
        ...
        svm.res = svm(x=train.dat,y=as.factor(bs_mat.resp[,bs]),kernel="radial",
        ...
        test.res = predict(svm.res,newdata=test.mat,decision.values = TRUE)
  ...
  tmp_str = paste("Single cancer",core.cancer[ca],"by SVM(Radial) with gamma",
                  param_mat[auc.best_ix,2],"and cost",param_mat[auc.best_ix,1],sep=" ")
  plot(prauc.best,main=tmp_str)
  #best.param_pr[ca,] = param_mat[auc.best_ix,]
  ...
  #best.param_roc[ca,] = param_mat[auc.best_ix,]
