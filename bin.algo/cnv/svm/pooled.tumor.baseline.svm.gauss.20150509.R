...
gammas = c(0.001,0.01,0.1,1,2,3,4,5,10,100)
K = length(costs)*length(gammas)
param_mat = matrix(0,nrow=K,ncol=2)
param_mat[,1] = rep(costs,each=length(gammas))
param_mat[,2] = rep(gammas,length(costs))
#BS = 100

#all.res = list()
pdf("performance.pancancer.pooled.baseline.svm.guass.20150513.pdf",height=7,width=14)
...
for(cv in 4:8)
{
  ...
  for(j in 1:K)
  {
    svm.res = svm(x=train.mat,y=as.factor(train.resp),kernel="radial",
                  cost=param_mat[j,1],gamma=param_mat[j,2],scale=FALSE)
    test.res = predict(object=svm.res,newdata=test.mat,decision.values = TRUE)
    curr.test_res[j,] = t(attributes(test.res)$decision.values)
  }
...
tmp_str = paste("Pan-cancer(pooled data) by SVM(Radial) with gamma",
                param_mat[auc.best_ix,2],"and cost",param_mat[auc.best_ix,1],sep=" ")
plot(prauc.best,main=tmp_str)
best.param_pr = param_mat[auc.best_ix,]
...
best.lambda_roc = param_mat[auc.best_ix,]
...
for(ca in 1:length(core.cancer))
{
  ...
  tmp_str = paste("Performance of best model(by AUC under PR curve) on",core.cancer[ca],sep=" ")
...
for(ca in 1:length(core.cancer))
{
  ...
  tmp_str = paste("Test on core cancer",core.cancer[ca]," by SVM(radial)",
                  "gamma =",param_mat[curr_auc.best_ix,2],"cost =",param_mat[curr_auc.best_ix,1],sep=" ")
  plot(prauc.best,main=tmp_str)
