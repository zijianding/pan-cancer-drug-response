...
#all.res = list()
pdf("performance.svm_linear.pancancer.pooled.baseline.20150510.pdf")
...
for(cv in 4:8)
{
  ...
  for(j in 1:K)
  {
    svm.res = svm(x=train.mat,y=as.factor(train.resp),kernel="linear",cost=costs[j],scale=FALSE,probability=TRUE)
    test.res = predict(object=svm.res,newdata=test.mat,decision.values = TRUE,probability=TRUE)
...
tmp_str = paste("Pan-cancer(pooled data) by SVM with cost",costs[auc.best_ix],sep=" ")
plot(prauc.best,main=tmp_str)
best.lambda = costs[auc.best_ix]
...
tmp_str = paste("Pan-cancer(pooled data) by SVM",costs[auc.best_ix],sep=" ")
plot(roc.best,main=tmp_str)
best.lambda_roc = costs[auc.best_ix]
...
for(ca in 1:length(core.cancer))
{
  ...
  tmp_str = paste("Test on core cancer",core.cancer[ca]," by SVM(",costs[curr_auc.best_ix],")",sep=" ")
  plot(prauc.best,main=tmp_str)
