...
train.resp = as.character(train.info$response[match(colnames(train.dat),
                                                    as.character(train.info$patient))])
...
for(i in 1:length(alphas))
{
                         ...
                         .init=list(list(), list()) ) %dopar%
  {
    ...
    #in case 0 deviance
    while( sum(curr.train_dat)==0 )
    {
      tmp.bs_mat = bootstrap_sample( colnames(train.dat), train.resp, BS=BS)
      tmp.pats = tmp.bs_mat[[1]]
      tmp.resp = tmp.bs_mat[[2]]
      
      curr.train_dat = train.dat[ ,match(tmp.pats[,bs],colnames(train.dat)) ]
      curr.train_resp = as.factor(tmp.resp[,bs])
      bs_mat.pats[,bs] = tmp.pats[,bs]
      bs_mat.resp[,bs] = tmp.resp[,bs]
...
tmp_str = paste(output_folder,"/single.elanat.test_performance.test_",test_fold-3,".20150701.tiff",sep="")
tiff(tmp_str)
#roc
...
title(paste("Test performance on ",core.cancer,sep=""),tmp_str)
