...
all.res = list()
pdf("performance.tree.single_cancer.20150513.pdf")
for(ca in 1:length(core.cancer))
{
  ...
  for(cv in 4:8)
  {
    ...
    for(bs in 1:BS)
    {
      ...
      train.resp = bs_mat.resp[,bs]
      tmp.train_df = data.frame(train.dat,train.resp)
    ...
    for(i in 1:length(test.pats))
    {
      ...
      score.mat[,test.ix] = mean(curr.mat)
    }
  }
  ...
  tmp_str = paste("Single cancer",core.cancer[ca],"by decision tree",sep=" ")
  plot(prauc.best,main=tmp_str)
  #best.lambdas[ca] = lambdas[auc.best_ix]
  ...
  #best.lambdas_roc[ca] = lambdas[auc.best_ix]
